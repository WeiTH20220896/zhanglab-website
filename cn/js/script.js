...
document.addEventListener('DOMContentLoaded', function() {
    // Floating language switch (EN / 中文)
    ...
    if (isCn) {
        enHref = path
            .replace('/cn/pages/', '/pages/')
            .replace('/cn/index.html', '/index.html')
            .replace('/cn/', '/');
        zhHref = path;
    } else {
        if (path.includes('/pages/')) {
            zhHref = path.replace('/pages/', '/cn/pages/');
        } else if (path.endsWith('/index.html')) {
            zhHref = path.replace('/index.html', '/cn/index.html');
        } else if (path.endsWith('/')) {
            zhHref = path + 'cn/index.html';
        } else {
            zhHref = '/cn/index.html';
        }
        enHref = path;
    }

    const langBox = document.createElement('div');
    langBox.setAttribute('aria-label', 'Language switch');
    langBox.style.position = 'fixed';
    langBox.style.right = '18px';
    langBox.style.bottom = '18px';
    langBox.style.zIndex = '1200';
    langBox.style.display = 'inline-flex';
    langBox.style.gap = '6px';
    langBox.style.padding = '6px';
    langBox.style.borderRadius = '12px';
    langBox.style.background = 'rgba(255, 255, 255, 0.95)';
    langBox.style.border = '1px solid rgba(26, 84, 144, 0.25)';
    langBox.style.boxShadow = '0 10px 24px rgba(26, 84, 144, 0.18)';

    const enBtn = document.createElement('a');
    enBtn.href = enHref;
    enBtn.textContent = 'EN';
    enBtn.style.textDecoration = 'none';
    enBtn.style.padding = '6px 10px';
    enBtn.style.borderRadius = '8px';
    enBtn.style.fontSize = '12px';
    enBtn.style.fontWeight = '700';
    enBtn.style.color = isCn ? '#1a5490' : '#ffffff';
    enBtn.style.background = isCn ? 'transparent' : '#1a5490';

    const zhBtn = document.createElement('a');
    zhBtn.href = zhHref;
    zhBtn.textContent = '中文';
    zhBtn.style.textDecoration = 'none';
    zhBtn.style.padding = '6px 10px';
    zhBtn.style.borderRadius = '8px';
    zhBtn.style.fontSize = '12px';
    zhBtn.style.fontWeight = '700';
    zhBtn.style.color = isCn ? '#ffffff' : '#1a5490';
    zhBtn.style.background = isCn ? '#1a5490' : 'transparent';

    langBox.appendChild(enBtn);
    langBox.appendChild(zhBtn);
